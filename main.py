...
AVL_CAT_NAME = "Name (Catalog Part)"
...
AVL_COMMODITY_CODE = "Linked Commodity Code (Manufacturer)"
AVL_CATS_REFERENTIAL = "CatS Referential(Catalog Part)"
...
def calculate(products, production_counts):
    ...
    # 同一Part Number在不同研发BOM中可能使用了不同或过期的Part Name。
    # 汇总必须只按Part Number进行，避免同一料号因名称不同而被拆成多行。
    # 此处先保留首次出现的研发名称作为临时值，导出前统一用AVL最新名称覆盖。
    part_name_map = (
        detail_df[[PART_COL, NAME_COL]]
        .drop_duplicates(subset=[PART_COL], keep="first")
    )
    wide_df = detail_df.pivot_table(
        index=PART_COL, columns="产品名称", values="无备损总需求",
        aggfunc="sum", fill_value=0, sort=False
    ).reset_index()
    wide_df = wide_df.merge(part_name_map, on=PART_COL, how="left")
    product_cols = [col for col in wide_df.columns if col not in [PART_COL, NAME_COL]]
    wide_df = wide_df[[PART_COL, NAME_COL] + product_cols]
    wide_df["总需求数量"] = wide_df[product_cols].sum(axis=1).astype(int)
    ...
    # 分产品明细同样只按Part Number和产品汇总，防止旧名称造成重复行。
    dist_df = detail_df.groupby(
        [PART_COL, "产品名称"], as_index=False, sort=False
    )["无备损总需求"].sum()
    dist_df = dist_df.merge(part_name_map, on=PART_COL, how="left")
    dist_df = dist_df[[PART_COL, NAME_COL, "产品名称", "无备损总需求"]]
...
def calculate_single_product_usage(products):
    """
    生成单个产品BOM usage矩阵：每个产品都按数量1计算。
    行为物料，列为产品，单元格为研发BOM中的单台/套Qty，不乘计划生产数量，
    也不计算备损。Part Number顺序按BOM加载顺序及首次出现顺序保留。
    """
    rows = []
    part_order = {}
    next_order = 0
    ...
    for prod_name, df in products.items():
        temp = df.copy()
        temp["产品名称"] = prod_name
        temp["单个产品Usage"] = pd.to_numeric(temp[QTY_COL], errors="coerce").fillna(0)
        for part_number in temp[PART_COL].tolist():
            key = standardize_pn(part_number)
            if key and key not in part_order:
                part_order[key] = next_order
                next_order += 1
        rows.append(temp[[PART_COL, NAME_COL, "产品名称", "单个产品Usage"]])
    if not rows:
        return None
    source = pd.concat(rows, ignore_index=True)
    # 仅以Part Number作为物料唯一键。研发名称不同不能把同一料号拆成多行。
    part_name_map = (
        source[[PART_COL, NAME_COL]]
        .drop_duplicates(subset=[PART_COL], keep="first")
    )
    result = source.pivot_table(
        index=PART_COL, columns="产品名称", values="单个产品Usage",
        aggfunc="sum", fill_value=0, sort=False
    ).reset_index()
    result = result.merge(part_name_map, on=PART_COL, how="left")
    for product_name in product_names:
        if product_name not in result.columns:
            result[product_name] = 0
    result["__研发BOM顺序"] = result[PART_COL].apply(
        lambda value: part_order.get(standardize_pn(value), len(part_order))
    )
    result = result.sort_values("__研发BOM顺序", kind="stable").drop(columns=["__研发BOM顺序"])
    return result[[PART_COL, NAME_COL] + product_names].reset_index(drop=True)
...
def load_avl(file_path):
    try:
        ...
        req_keys = [
            "number(catalogpart)", "name(catalogpart)",
            "number(manufacturerpart)", "name(manufacturer)",
            "availability(manufacturerpart)", "sourcingstatus(amllink)",
            "linkedcommoditycode(manufacturer)", "catsreferential(catalogpart)"
        ]
        ...
        use_df.columns = [
            AVL_CAT_PART, AVL_CAT_NAME, AVL_MF_PART, AVL_MF_NAME,
            AVL_AVAIL, AVL_SOURCE_STATUS, AVL_COMMODITY_CODE, AVL_CATS_REFERENTIAL
        ]
...
def build_avl_catalog_name_map(avl_df):
    """按Catalog Part生成唯一的最新Catalog Name映射，避免多条AML记录放大行数。"""
    if avl_df is None or AVL_CAT_PART not in avl_df.columns or AVL_CAT_NAME not in avl_df.columns:
        return pd.DataFrame(columns=[AVL_CAT_PART, AVL_CAT_NAME])
    name_map = avl_df[[AVL_CAT_PART, AVL_CAT_NAME]].copy()
    name_map[AVL_CAT_PART] = name_map[AVL_CAT_PART].apply(standardize_pn)
    name_map[AVL_CAT_NAME] = name_map[AVL_CAT_NAME].fillna("").astype(str).str.strip()
    name_map = name_map[(name_map[AVL_CAT_PART] != "") & (name_map[AVL_CAT_NAME] != "")]
    return name_map.drop_duplicates(subset=[AVL_CAT_PART], keep="first")


def refresh_part_name_from_avl(df, avl_df):
    """
    仅用于“Usage+AVL匹配”导出模式：按Part Number匹配AVL的Name (Catalog Part)。
    AVL名称有效时覆盖研发Part Name；未匹配或AVL名称为空时保留研发原名称。
    """
    result = df.copy()
    if PART_COL not in result.columns or NAME_COL not in result.columns:
        return result
    result[PART_COL] = result[PART_COL].apply(standardize_pn)
    name_map = build_avl_catalog_name_map(avl_df)
    if name_map.empty:
        return result
    result = result.merge(name_map, left_on=PART_COL, right_on=AVL_CAT_PART, how="left")
    valid_name = result[AVL_CAT_NAME].notna() & result[AVL_CAT_NAME].astype(str).str.strip().ne("")
    result.loc[valid_name, NAME_COL] = result.loc[valid_name, AVL_CAT_NAME].astype(str).str.strip()
    return result.drop(columns=[AVL_CAT_PART, AVL_CAT_NAME], errors="ignore")


def enrich_single_product_usage_from_avl(df, avl_df):
    """
    为“单个产品BOM usage”追加AVL字段，并固定放在Part Name后、产品列前：
    1. Linked Commodity Code (Manufacturer)
    2. CatS Referential(Catalog Part)
    同一Catalog Part在AVL中存在多行时，各字段取第一条非空值，避免放大Usage行数。
    """
    result = refresh_part_name_from_avl(df, avl_df)
    if result is None or PART_COL not in result.columns:
        return result

    required = [AVL_CAT_PART, AVL_COMMODITY_CODE, AVL_CATS_REFERENTIAL]
    if avl_df is None or any(col not in avl_df.columns for col in required):
        for col in [AVL_COMMODITY_CODE, AVL_CATS_REFERENTIAL]:
            if col not in result.columns:
                result[col] = ""
    else:
        source = avl_df[required].copy()
        source[AVL_CAT_PART] = source[AVL_CAT_PART].apply(standardize_pn)
        ...
        avl_map = source.groupby(AVL_CAT_PART, as_index=False, sort=False).agg({
            AVL_COMMODITY_CODE: first_non_empty,
            AVL_CATS_REFERENTIAL: first_non_empty
        })
        result[PART_COL] = result[PART_COL].apply(standardize_pn)
        result = result.merge(avl_map, left_on=PART_COL, right_on=AVL_CAT_PART, how="left")
        result.drop(columns=[AVL_CAT_PART], errors="ignore", inplace=True)
        result[AVL_COMMODITY_CODE] = result[AVL_COMMODITY_CODE].fillna("")
        result[AVL_CATS_REFERENTIAL] = result[AVL_CATS_REFERENTIAL].fillna("")

    product_cols = [
        col for col in result.columns
        if col not in [PART_COL, NAME_COL, AVL_COMMODITY_CODE, AVL_CATS_REFERENTIAL]
    ]
    return result[
        [PART_COL, NAME_COL, AVL_COMMODITY_CODE, AVL_CATS_REFERENTIAL] + product_cols
    ]


def merge_avl_info(main_df, avl_df):
    main_df = refresh_part_name_from_avl(main_df, avl_df)
    ...
    result.drop(columns=[AVL_CAT_PART, AVL_CAT_NAME], errors="ignore", inplace=True)
...
def beautify_single_product_usage(ws, production_counts=None):
    """
    将单个产品BOM usage排成横向矩阵：
    第1行为Part Number、Part Name及各产品名称；数据行为产品数量=1时的物料用量。
    不再显示实际计划生产数量行。
    """
    for col_idx in range(1, ws.max_column + 1):
...
def export_frames(path, frames, supplier_source_df=None, production_counts=None):
    used = set()
    unit_sheet_name = None
    with pd.ExcelWriter(path, engine="openpyxl") as writer:
        for sheet_name, df in frames:
            safe_name = safe_sheet_name(sheet_name, used)
            df.to_excel(writer, sheet_name=safe_name, index=False)
            if sheet_name == "单个产品BOM usage":
                unit_sheet_name = safe_name
    wb = load_workbook(path)
    for ws in wb.worksheets:
        if ws.title == unit_sheet_name:
            beautify_single_product_usage(ws, production_counts or {})
        else:
...
class BOMAVLApp(Tk):
    def __init__(self):
        ...
        Button(switch_frame, text="单个产品BOM usage", command=lambda: self.show_table(self.unit_wide_df), width=20, bg="#9C27B0", fg="white").grid(row=0, column=1, padx=4)
        Button(switch_frame, text="分产品明细", command=lambda: self.show_table(self.dist), width=14, bg="#9C27B0", fg="white").grid(row=0, column=2, padx=4)
    ...
    def do_calc(self):
        try:
            self.production_counts = self.get_prod_counts()
            self.detail, self.dist, self.wide_df = calculate(self.products, self.production_counts)
            # 单个产品BOM usage只显示各产品数量=1时的研发BOM单台/套Qty，
            # 不显示实际计划生产数量，也不乘备损率。
            self.unit_wide_df = calculate_single_product_usage(self.products)
    ...
    def do_export(self):
        ...
        if self.avl_df is None:
            messagebox.showwarning(
                "提示",
                "所有导出表的Part Name都需要按照AVL中的Name (Catalog Part)更新，请先加载AVL文件。"
            )
            return
        path = filedialog.asksaveasfilename(
        ...
        try:
            frames = []
            # 所有导出表都按Part Number匹配AVL的Number (Catalog Part)，
            # 并使用Name (Catalog Part)更新Part Name。未匹配或AVL名称为空时保留研发原名称。
            wide_named = refresh_part_name_from_avl(self.wide_df, self.avl_df)
            unit_wide_full = enrich_single_product_usage_from_avl(self.unit_wide_df, self.avl_df)
            dist_named = refresh_part_name_from_avl(self.dist, self.avl_df)
            product_frames_named = [
                (prod_name, refresh_part_name_from_avl(df, self.avl_df))
                for prod_name, df in self.products.items()
            ]

            if mode == 2:
                # “不带AVL”表示不附加AVL制造商等字段，但Part Name仍使用AVL最新Catalog Name。
                wide_without_avl = wide_named
                wide_with_avl = merge_avl_info(self.wide_df, self.avl_df)
                flag_map = wide_with_avl[[PART_COL, AVL_FLAG_COL]].drop_duplicates(subset=[PART_COL])
                dist_full = dist_named.merge(flag_map, on=PART_COL, how="left")
                no_avl_df = wide_with_avl[wide_with_avl[AVL_FLAG_COL] == NO_AVL_TEXT].copy()
                frames.extend([
                    ("总BOM usage_不带AVL", wide_without_avl),
                    ("总BOM usage_带AVL", wide_with_avl),
                    ("单个产品BOM usage", unit_wide_full),
                    ("无AVL物料", no_avl_df),
                    ("分产品明细", dist_full)
                ])
                wide_full = wide_with_avl
                avl_info = f"\n无AVL物料：{no_avl_df[PART_COL].nunique()} 个，已单独输出到“无AVL物料”Sheet。"
            else:
                # 仅导出Usage不附加AVL字段，但所有Part Name仍按AVL最新Catalog Name更新。
                wide_full = wide_named
                frames.extend([
                    ("总BOM usage", wide_named),
                    ("单个产品BOM usage", unit_wide_full),
                    ("分产品明细", dist_named)
                ])
                avl_info = ""
            frames.extend(product_frames_named)
            supplier_count = export_frames(
                path, frames, supplier_source_df=self.wide_df,
            ...
            messagebox.showinfo(
                ...
                f"所有Part Name已按AVL的Name (Catalog Part)更新。\n"
                f"单个产品BOM usage已增加Linked Commodity Code和CatS Referential。"
            )
        except PermissionError:
            messagebox.showerror("保存失败", "文件正被Excel占用，请关闭后重试。")
        except Exception as exc:
