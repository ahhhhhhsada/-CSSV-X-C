...
def calculate(products, production_counts):
    ...
    # 合并原“分产品明细”和“各产品计算明细”：
    # 同一个产品内相同Part Number只保留一行，同时汇总单台用量和需求数量。
    product_detail_df = detail_df.groupby(
    ...
    product_detail_df.rename(columns={
        QTY_COL: "单台用量 Qty",
        "无备损总需求": "单产品无备损总需求"
    }, inplace=True)
    product_detail_df["单产品无备损总需求"] = (
        product_detail_df["单产品无备损总需求"].round().astype(int)
    )

    # 按研发BOM首次出现的Part Number顺序排列合并明细。
    product_detail_df["__研发BOM顺序"] = product_detail_df[PART_COL].apply(
        lambda value: original_part_order.get(standardize_pn(value), fallback_order)
    )
    product_detail_df = product_detail_df.sort_values(
        ["__研发BOM顺序", "产品名称"], kind="stable"
    ).drop(columns=["__研发BOM顺序"]).reset_index(drop=True)

    return detail_df, product_detail_df, wide_df
...
class BOMAVLApp(Tk):
    def __init__(self):
        ...
        self.products = {}
        self.entries = {}
        self.detail = None              # 原始逐行计算数据（内部保留）
        self.product_detail = None      # 合并后的分产品计算明细
        self.dist = None                # 兼容旧变量，指向product_detail
        self.wide_df = None
        ...
        Radiobutton(export_mode_frame, text="仅导出不带AVL的Usage", variable=self.export_mode, value=1).pack(side="left", padx=8)
        Radiobutton(export_mode_frame, text="同时导出带AVL和不带AVL", variable=self.export_mode, value=2).pack(side="left", padx=8)
        ...
        Button(
            switch_frame, text="总BOM usage_不带AVL",
            command=lambda: self.show_table(self.wide_df), width=22,
            bg="#9C27B0", fg="white"
        ).grid(row=0, column=0, padx=4)
        Button(
            switch_frame, text="总BOM usage_带AVL",
            command=self.show_wide_with_avl, width=22,
            bg="#7B1FA2", fg="white"
        ).grid(row=0, column=1, padx=4)
        Button(
            switch_frame, text="分产品计算明细",
            command=lambda: self.show_table(self.product_detail), width=18,
            bg="#9C27B0", fg="white"
    ...
    def do_calc(self):
        try:
            self.detail, self.product_detail, self.wide_df = calculate(
                self.products, self.get_prod_counts()
            )
            self.dist = self.product_detail
    ...
    def show_wide_with_avl(self):
        """在界面中预览带完整AVL信息的总BOM usage。"""
        if self.wide_df is None:
        ...
        if self.avl_df is None:
            ...
            return
        self.show_table(merge_avl_info(self.wide_df, self.avl_df))
    ...
    def do_export(self):
        ...
        if mode == 2 and self.avl_df is None:
            messagebox.showwarning(
                "提示",
                "已选择“导出Usage+AVL匹配”，请先加载AVL文件，或切换为仅导出Usage。"
            )
            return
        ...
        path = filedialog.asksaveasfilename(
            ...
            initialfile=(
                "一体化备料汇总.xlsx"
                if mode == 1 else "一体化备料汇总_含两种BOM_usage.xlsx"
            )
        )
        if not path:
            return
        ...
        try:
            ...
            # 第一张总表始终为纯需求数据，不含任何AVL字段。
            frames.append(("总BOM usage_不带AVL", self.wide_df.copy()))

            if mode == 2:
                # 第二张总表包含完整AVL信息；一对多AVL关系会保留多行。
                wide_with_avl = merge_avl_info(self.wide_df, self.avl_df)
                frames.append(("总BOM usage_带AVL", wide_with_avl))

                # 无AVL物料继续单独输出，并保持研发BOM相对顺序。
                no_avl_df = wide_with_avl[
                    wide_with_avl[AVL_FLAG_COL] == NO_AVL_TEXT
                ].copy()
                frames.append(("无AVL物料", no_avl_df))
                no_avl_count = no_avl_df[PART_COL].nunique()

                # 合并后的明细仅增加“是否存在AVL”状态，不放大明细行数。
                flag_map = wide_with_avl[[PART_COL, AVL_FLAG_COL]].copy()
                flag_map["__AVL优先级"] = flag_map[AVL_FLAG_COL].apply(
                    lambda value: 1 if value == "存在AVL匹配信息" else 0
                )
                flag_map = flag_map.sort_values(
                    "__AVL优先级", ascending=False, kind="stable"
                ).drop_duplicates(subset=[PART_COL], keep="first")
                flag_map.drop(columns=["__AVL优先级"], inplace=True)
                product_detail_export = self.product_detail.merge(
                    flag_map, on=PART_COL, how="left"
                )
                product_detail_export[AVL_FLAG_COL] = (
                    product_detail_export[AVL_FLAG_COL].fillna(NO_AVL_TEXT)
                )
            else:
                product_detail_export = self.product_detail.copy()

            # 用一张“分产品计算明细”替代原来的两张重叠明细表。
            frames.append(("分产品计算明细", product_detail_export))

            # 各产品清洗后的原始BOM继续保留。
            frames.extend((prod_name, df) for prod_name, df in self.products.items())

            # Material Preparation仍固定在最后一个Sheet，数据来源是不带AVL总表。
            supplier_count = export_frames(
                path, frames, supplier_source_df=self.wide_df
            )

            mode_info = (
                f"已同时生成不带AVL和带AVL两张总BOM usage。\n"
                f"无AVL物料：{no_avl_count} 个。\n"
                if mode == 2 else
                "当前为仅Usage模式，已生成不带AVL总BOM usage。\n"
            )
            messagebox.showinfo(
                "导出完成",
                f"文件保存成功！\n"
                f"总物料数：{self.wide_df[PART_COL].nunique()}\n"
                f"{mode_info}"
                f"原两张明细已合并为“分产品计算明细”。\n"
                f"最后一个Sheet“{SUPPLIER_SHEET_NAME}”：{supplier_count} 个物料。\n"
